...
REQUEST_LATENCY = Histogram(
    "django_request_latency_seconds", "Latence des requêtes", ["endpoint"]
)
...
class InvalidRequestLoggerMiddleware:
    def __init__(self, get_response):
        self.get_response = get_response
    ...
    def __call__(self, request):
        ...
        if response.status_code >= 400:
            ...
            logger.error(
                f"Invalid HTTP request: {request.path} returned {response.status_code}"
            )
        return response
