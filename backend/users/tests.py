...
class UserUpdateViewTest(APITestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="testuser", password="testpassword123", display_name="OriginalName"
        )
        self.url = reverse("update")

        # Obtain a JWT token for the test user
        response = self.client.post(
            "/api/users/token/", {"username": "testuser", "password": "testpassword123"}
        )
...
class FriendshipTests(TestCase):
    def setUp(self):
        ...
        Friendship.objects.create(
            requester=self.user1, receiver=self.user2, status="accepted"
        )
        Friendship.objects.create(
            requester=self.user2, receiver=self.user3, status="sent"
        )
        self.client = APIClient()
