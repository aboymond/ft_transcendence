...
import { textStylePVPMenu2, textStyleTournamentMenu, defaultColor } from '../index';

export const SceneMenu2: React.FC<{ loadScene: (sceneName: string) => void }> = ({ loadScene }) => {
	const [currentSelect, setCurrentSelect] = useState(0);

	const handleKeyDown = (e: KeyboardEvent) => {
		if (e.code === 'ArrowUp') {
			setCurrentSelect((prevSelect) => (prevSelect > 0 ? prevSelect - 1 : 2));
		}
		if (e.code === 'ArrowDown') {
			setCurrentSelect((prevSelect) => (prevSelect < 2 ? prevSelect + 1 : 0));
		}
		if (e.code === 'Enter') {
			// loadScene('SceneGame'); // TODO
		}
	};
	...
	console.log('Rendering SceneMenu2');

	return (
		<SceneBase>
			<Text
				text="Hello, SceneMenu2!"
				style={textStylePVPMenu2}
			...
			<Graphics
				draw={(g) => {
					g.clear();
					if (currentSelect === 0) {
						g.beginFill(0x053100);
					} else {
						g.beginFill(defaultColor);
					}
					g.drawRect(0, 0, window.innerWidth, window.innerHeight / 3);
					g.endFill();
				}}
			/>
			<Text text="TOURNAMENT" style={currentSelect === 0 ? textStyleTournamentMenu : textStylePVPMenu2} />
			{/* Add the other Text and Graphics components here */}
		</SceneBase>
	);
};
